...
var Main = function() {
	this.ccTypeArray = [art_CCHanddrawn,art_CCGraffiti,art_CCDrips,art_CCSpatter,art_CCSpatten,art_CCCurve];
	var _gthis = this;
...
Sketcher.prototype = {
	appendTo: function(element) {
		...
		case "webgl":
			...
			break;
		default:
			haxe_Log.trace("case '" + this.settings.get_type().toLowerCase() + "': trace ('" + this.settings.get_type().toLowerCase() + "');",{ fileName : "Sketcher.hx", lineNumber : 115, className : "Sketcher", methodName : "appendTo"});
		}
		return this;
	}
	...
	,update: function() {
		...
		case "svg":
			...
			break;
		case "webgl":
			haxe_Log.trace("webgl",{ fileName : "Sketcher.hx", lineNumber : 639, className : "Sketcher", methodName : "update"});
			var _g3 = 0;
			...
			break;
		default:
			haxe_Log.trace("case '" + this.settings.get_type() + "': trace ('" + this.settings.get_type() + "');",{ fileName : "Sketcher.hx", lineNumber : 648, className : "Sketcher", methodName : "update"});
		}
		this.baseArray = [];
	}
	,__class__: Sketcher
	...
	_keyDown: function(e) {
		...
		if(e.metaKey == true && (e.code == "KeyS" && e.altKey == true)) {
			...
			if(this.sketch.settings.get_type() == "svg") {
				...
				window.console.warn("no canvas-2-svg");
			}
		}
...
$hxClasses["art.CCGraffiti"] = art_CCGraffiti;
art_CCGraffiti.__name__ = "art.CCGraffiti";
art_CCGraffiti.__super__ = SketcherBase;
art_CCGraffiti.prototype = $extend(SketcherBase.prototype,{
	setupVideo: function() {
	...
	,nextColor: function() {
		this._colorCounter++;
		if(this._colorCounter >= this._colorArr.length) {
			this._colorCounter = 0;
		}
		this._activeColor = this._colorArr[this._colorCounter];
	}
	,initDatGui: function() {
		var _gthis = this;
		var guiSettings = { name : "" + this.toString(), closed : false};
		var gui = new dat.gui.GUI(guiSettings);
		gui.add(this,"message");
	...
	,setup: function() {
		...
		this._colorArr = [this._color1,this._color2,this._color3,this._color4];
		this._activeColor = this._colorArr[this._colorCounter];
		this.isDebug = true;
		this.message = this.toString();
		this.description = this.toString();
...
art_CCSpatten.prototype = $extend(SketcherBase.prototype,{
	...
	,setup: function() {
		haxe_Log.trace("SETUP :: " + this.toString(),{ fileName : "src/art/CCSpatten.hx", lineNumber : 63, className : "art.CCSpatten", methodName : "setup"});
		this.message = this.toString();
		this.description = this.toString();
		this.grid = new sketcher_util_GridUtil(Globals.w,Globals.h);
		this.grid.setIsCenterPoint(true);
...
sketcher_draw_Base.prototype = {
	...
	,clone: function() {
		haxe_Log.trace("WIP",{ fileName : "sketcher/draw/Base.hx", lineNumber : 251, className : "sketcher.draw.Base", methodName : "clone"});
...
sketcher_draw_Polygon.prototype = $extend(sketcher_draw_Base.prototype,{
	...
	,getPoint: function(id) {
		if(id * 2 > this.get_arr().length) {
			haxe_Log.trace("not in this length",{ fileName : "sketcher/draw/Polygon.hx", lineNumber : 135, className : "sketcher.draw.Polygon", methodName : "getPoint"});
		}
